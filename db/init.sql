...
CREATE TABLE IF NOT EXISTS products (
    ...
    zona ENUM('kiosko', 'vip', 'ambos') NOT NULL DEFAULT 'ambos',
...
SET @products_zona_exists := (
    ...
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'products' AND COLUMN_NAME = 'zona'
);
...
SET @products_zona_index_exists := (
    ...
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'products' AND INDEX_NAME = 'idx_products_zona'
);
...
/* =========================================================
   TABLAS VIP: ventas y cierres independientes
   ========================================================= */
...
CREATE TABLE IF NOT EXISTS vip_sales (
    ...
    CONSTRAINT fk_vip_sales_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS vip_closings (
    ...
    CONSTRAINT fk_vip_closings_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
